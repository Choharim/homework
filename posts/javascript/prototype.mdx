...
모든 객체는 _ _proto_ _ 접근자 프로퍼티로 내부 슬롯에 있는 자신의 프로토타입에 간접적으로 접근할 수 있다.  _ _proto_ _ 접근자 프로퍼티는 객체가 직접 소유한 프로퍼티가 아닌 Object.prototype 프로퍼티이다. 
...
자바스크립트 엔진은 `객체의 프로퍼티 또는 메소드에 접근하려고 할 때` `해당 객체에 없으면` 해당 객체의 `[[ Prototype ]] 내부 슬롯에 저장된 참조` 즉,  _ _proto_ _ 접근자 프로퍼티가 가리키는 참조를 `따라 상위 프로토타입의 프로퍼티를 순차적으로 검색한다.` 이를 프로토타입 체인이라 한다. 
...
2. person.__proto__ (= Person.prototype)에 hasOwnProperty 프로퍼티가 없다.
3. person.__proto__.__proto__ (= Object.prototype)에 hasOwnProperty 프로퍼티가 있다.
