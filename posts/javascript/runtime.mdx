...
- 불필요한 task가 Call Stack에 많이 적재되어있으면 Render Queue에 있는 task가 처리되지 못해 렌더링 되지 못한다.
...
3. `() => {
        ...
        console.log('Listener 1');
      }` 첫 번째 이벤트 리스너의 callback이 Task Queue에 push 된다.
4. `() => {
        Promise.resolve().then(() => {
          console.log('Microtask 2');
        });
        console.log('Listener 2');
      }` 두 번째 이벤트 리스너의 callback이 Task Queue에 push 된다.
