...
Frequency Counter을 이용하면 one step으로 접근하여 확인할 수 있기 때문에 시간 복잡도가 `O(n)`으로 대폭 줄어든다.
...
function charCount(words) {
  ...
  for (const char of words) {
    ...
    object[char] = (object[char] ?? 0) + 1 // undefined이면 1, 이전 값이 존재하면 +1
  }
...
> 문제 2 - 문제 1와 동일하되, key는 소문자로 지정하고 숫자는 무시한다.

[풀이]

1. key에 해당하지 않는 문자는 제거한 후 소문자로 변경한다.
2. 해당 value가 undefined이면 기본값 0을 지정하고 값이 존재하면 이전값을 이용하여 1씩 카운트를 증가시킨다.
...
function charCount(words) {
  ...
  for (const char of words) {
    ...
    object[key] = (object[key] ?? 0) + 1 // undefined이면 1, 이전 값이 존재하면 +1
  }
...
toLowerCase는 O(n)이지만 char.toLowerCase()부분은 1개의 문자를 소문자로 상황이므로 시간 복잡도는 O(1)이다.  
...
2. 각 배열의 frequency counter(각 array의 요소를 key로 갖고 해당 요소의 갯수를 value로 갖는 object)를 생성한다.
3. 한개의 object를 기준으로 다른 object에 같은 키 값이 존재하지 않거나, 다른 값을 갖는다면 false를 리턴한다.
...
> 문제 3 - 함수의 인수에 중복된 숫자또는 문자가 존재하는지 확인해라  
> ex)  
...
1. object를 생성한다.
2. arguments를 순회하며 object에 해당 요소를 key로 첫 value는 true로 지정한다.
3. value가 true이면 이전에 object에 저장되었다는 것이므로 true를 리턴한다.
4. 그외는 false를 리턴한다.
