...
<Picture src="merge-sort.png" alt="합병 정렬 트리" height={400} />

## 병합

> 정렬된 두 배열을 정렬하며 합치기 위해서 앞 요소부터 작은 값을 찾으며 병합해간다.

정렬된 두 배열을 정렬(오름차순)하며 합쳐보자.
...
> 1개의 요소만 갖는 배열이 될 때까지 절반으로 배열을 나누고, 절반으로 나눠진 배열을 병합해간다.

1. 절반으로 나눈 배열을 mergeSortedArray 인수로 넣어 합쳐준다.
2. mergeSort 인수로 배열 길이 1일 때 더이상 배열을 절반으로 나누지 않아도 되므로 해당 배열을 리턴하며 mergeSort 호출을 멈춘다.
3. 더이상 재귀 함수가 호출되지 않으면 직전에 반으로 나눠진 배열 두개가 merge되고 이를 반복하여 최종 1개로 병합된다.
