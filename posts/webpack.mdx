...
- css 번들링
  css-loader는 js 코드로 변환해주고 → style-loader는 그 js 코드를 DOM에 적용해준다. - css-loader
  ...
  모듈로 변경된 스타일 시트는 돔에 추가되어야만 브라우져가 해석할 수 있다. css 코드는 CSSOM으로 바뀌어야 스타일이 적용될 수 있다. CSSOM으로 적용하기 위해서는 html에 css파일을 직접 불러오거나 인라인 스크립트로 직접 넣어주어야 한다. - style-loader
...
- clean option
  ...
  `jsx // webpack.config.js { output: { path: path.resovle(__dirname, 'dist'), clean: true } } `
