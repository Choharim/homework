...
makeCounter 함수가 호출되면 makeCounter 함수의 실행 컨텍스트가 생성되고 내부 함수의 정의가 평가된다. 이때, 내부 함수의 내부 슬롯에 상위 렉시컬 환경 참조 값을 저장하여 기억한다. 변수에 makeCounter 함수의 내부 함수를 반환하면 makeCounter함수의 실행 컨텍스트는 스택에서 제거되고, 변수 counter1과 counter2에 담긴 내부함수는 서로 다른 상위 렉시컬 환경 참조 값을 저장하고 있으므로 내부 변수가 공유되지 않고 독립적으로 유지될 수 있다.
